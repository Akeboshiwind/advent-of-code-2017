...
(defn count-jumps
  ...
  (let [instructions (parse instructions)
        inst-length (dec (count instructions))]
    (loop [instructions instructions
           pointer 0
           acc 0]
      (if (or (> pointer inst-length)
              (< pointer 0))
        acc
...
(u/test-examples count-jumps examples-pt1)
;; => true
  ...
  (let [instructions (parse instructions)
    ...
    (loop [instructions instructions
      ...
      (if (or (> pointer inst-length)
              (< pointer 0))
        acc
        (recur (update instructions pointer (fn [offset]
                                              (if (>= offset 3)
                                                (dec offset)
                                                (inc offset))))
               (+ pointer (get instructions pointer))
...
;; Test the test inputs
(u/test-examples count-jumps-2 examples-pt2)
;; => true
