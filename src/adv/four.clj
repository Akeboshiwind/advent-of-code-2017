...
(defn parse
  "Parses a passphrase into a list of words"
  [passphrase]
  (str/split passphrase #" "))

(defn one?
  "True if n is one, false otherwise"
  [n]
  (= n 1))

(defn valid?
  "True if passphrase is a valid password, i.e has no repeating words."
  [passphrase]
  (->> passphrase
       (parse)
       (frequencies)
...
(u/test-examples valid? examples-pt1)
;; => true
...
(defn count-valid
  "Count the passphrases that pass the predicate."
  [pred passphrases]
  (->> passphrases
       (str/split-lines)
       (filter pred)
...
(defn valid-2?
  "True if the passphrase contains no words that can be rearranged into each other."
  [passphrase]
  (->> passphrase
       (parse)
       (group-by sort)
...
;; Test the test inputs
(u/test-examples valid-2? examples-pt2)
;; => true

;; Now for the actual input
(count-valid valid-2? input)
;; => 167
